...
async def get_pw_info(original_url, token):
    try:
        # 1. Extract base URL - split on both ? and & and convert .mpd to .m3u8
        base_url = re.split(r'[?&]', original_url)[0].replace(".mpd", ".m3u8")
        ...
        # Using requests to ensure case-sensitive headers (client-type: WEB)
        resp = requests.post(
        ...
        if resp.status_code != 200:
            print(f"Penpencil API error: {resp.status_code}")
            return None, None
        ...
        # 3. Extract cookie values using parse_qs
...
def create_cookie_file(filename, policy, keypair, signature):
    # Netscape format requires TAB characters between fields
    lines = [
        f".cloudfront.net\tTRUE\t/\tTRUE\t0\tCloudFront-Policy\t{policy}",
        f".cloudfront.net\tTRUE\t/\tTRUE\t0\tCloudFront-Key-Pair-Id\t{keypair}",
        f".cloudfront.net\tTRUE\t/\tTRUE\t0\tCloudFront-Signature\t{signature}",
        f".pw.live\tTRUE\t/\tTRUE\t0\tCloudFront-Policy\t{policy}",
        f".pw.live\tTRUE\t/\tTRUE\t0\tCloudFront-Key-Pair-Id\t{keypair}",
        f".pw.live\tTRUE\t/\tTRUE\t0\tCloudFront-Signature\t{signature}"
    ]
